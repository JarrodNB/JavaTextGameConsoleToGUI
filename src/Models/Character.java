...
public class Character extends Observable implements Serializable {
    ...
    private void setMaxHealth(int hp) throws CharacterException {
        if (hp < 5) {
            throw new CharacterException("max hp is too low" + getClass().getSimpleName());
        }
        maxHealth = hp;
    }
    ...
    private void setBaseDefense(int defense) throws CharacterException {
        if (defense < 0) {
            throw new CharacterException("base defense cannot be less then 0" + getClass().getSimpleName());
        }
    ...
    public void setGold(int gold) throws CharacterException {
        if (gold < 0) {
            throw new CharacterException("gold can not be less then 0" + getClass().getSimpleName());
        }
        this.gold = gold;
        setChanged();
        notifyObservers();
    ...
    private void setBaseAttack(int attack) throws CharacterException {
        if (attack < 0) {
            throw new CharacterException("base attack can not be less then 0" + getClass().getSimpleName());
        }
