...
public class Weapon extends Item implements Serializable {
    ...
    public Weapon(Weapon weapon) throws WeaponException, ItemException {
        ...
        this.setUpgradeLevel(weapon.getUpgradeLevel());
        this.setUpgradeCost(weapon.getUpgradeCost());
    }
    ...
    private void setUpgradeAmount(int upgradeAmount) throws WeaponException {
        if (upgradeAmount < 1) {
            throw new WeaponException("weapon upgrade amount must be atleast 1" + getClass().getSimpleName());
        }
    ...
    private void setUpgradeLevel(int upgradeLevel) throws WeaponException {
        if (upgradeLevel < 0) {
            throw new WeaponException("weapon upgrade level must not be negative" + getClass().getSimpleName());
        }
    ...
    private void setUpgradeCost(int upgradeCost) throws WeaponException {
        if (upgradeCost < 1) {
            throw new WeaponException("upgrade cost must be atleast 1" + getClass().getSimpleName());
        }
