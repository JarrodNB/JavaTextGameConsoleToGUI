...
public class Item implements Serializable {
    ...
    public Item(Item item) throws ItemException{
        this(item.getId(), item.getName(), item.getDescription(), item.getBuyValue(), 1);
    }
    ...
    private void setId(int id) throws ItemException {
        if (id < 0) {
            throw new ItemException("Id can not be negative" + getClass().getSimpleName());
        }
        this.id = id;
    }
    ...
    private void setBuyValue(int buyValue) throws ItemException {
        if (buyValue < 0) {
            throw new ItemException("Buy value can not be negative" + getClass().getSimpleName());
        }
    ...
    public void setQuantity(int quantity) throws ItemException {
        if (quantity < 0) {
            throw new ItemException("Item quantity can not be negative" + getClass().getSimpleName());
        }
